...
contract MyBoard is Ownable{
    ...
    modifier onlyActiveUser(address user){require(user!=address(0x0),"Not a Registered User!");_;}
    ...
    event voteToPost(uint256 channelId,uint256 postId,uint256 upvotes,uint256 downvotes,uint256 votecount,address user,voteStatus votedstate,uint256 timestamp ); 
    event pollVote(uint256 channelId,uint256 postId,uint256 votenum,address user,uint256 pollid,uint256 timestamp);
    event newCommentCreated(uint256 channelId,uint256 postId, uint256 commentId,uint256 parentId, uint256 parentcommentcount,address postcreator,address creator,uint256 timestamp);
    ...
    event voteToComment(uint256 channelId,uint256 postId,uint256 commentId,uint256 upvotes,uint256 downvotes,uint256 votecount,address user,voteStatus votedstate,uint256 timestamp );
...
*/  
    ...
    function createPost(uint256 _channelId,string memory _privateKey, string memory title,string memory _contentUrl,string memory _encryptkey, postTypes _postType,postSorts _postSort,uint256 _pollCount,uint256 _expireTime)
     public  onlyActiveUser(_msgSender()) onlyActiveChannel(_channelId){    
        ...
        posts[id]=Post(_channelId,id,msg.sender,title,_contentUrl,_encryptkey,0,0,0,0,_postType,_postSort,0,block.timestamp );
    ...
    function postVoteUpSet(uint256 _id) public onlyActiveUser(_msgSender()) onlyActivePost(_id){ 
        ...
        if(postVote[msg.sender][_id]==voteStatus.None){
            posts[_id].upvotes=posts[_id].upvotes.add(1);
            postVote[msg.sender][_id]=voteStatus.VoteUp;  
        }else if(postVote[msg.sender][_id]==voteStatus.VoteDown){
            posts[_id].upvotes=posts[_id].upvotes.add(1);
             posts[_id].downvotes=posts[_id].downvotes.sub(1);
            postVote[msg.sender][_id]=voteStatus.VoteUp;           
        }
        else{
            posts[_id].upvotes=posts[_id].upvotes.sub(1);
            postVote[msg.sender][_id]=voteStatus.None;                    
        }
             
        emit voteToPost( posts[_id].channelId,_id, posts[_id].upvotes, posts[_id].downvotes,posts[_id].votecount,msg.sender,postVote[msg.sender][_id],block.timestamp  ); 
    }
    ...
    function postVoteDownSet(uint256 _id) public onlyActiveUser(_msgSender()) onlyActivePost(_id){ 
        ...
        posts[_id].votecount= posts[_id].votecount.add(1);    
         if(postVote[msg.sender][_id]==voteStatus.None){
            posts[_id].downvotes=posts[_id].downvotes.add(1);
    ...
    view returns (bool paidstate,voteStatus voteState,string memory channelName,uint256 userAwards,string memory privatekey) { 
        string memory _privatekey='';
        if(msg.sender==posts[_id].creator){
            _privatekey=privatePostKey[_id];
        }

        return (paidPostReadPermitted[msg.sender][_id],postVote[msg.sender][_id],
        channels[posts[_id].channelId].name,userPostAwards[msg.sender][_id],_privatekey);
    }
...
*/ 
    ...
    function createComment(uint256 _postid,string memory _contentUrl,string memory _encryptkey,uint256 _commentid,string memory _privatekey) public  onlyActiveUser(_msgSender())  onlyActivePost(_postid){
        ...
        comments[id]=Comment(id, msg.sender, _postid, _contentUrl,_encryptkey,0,0, 0,0,_commentid,0, block.timestamp);       
        if(_commentid>0){
          comments[_commentid].subcommentcount=comments[_commentid].subcommentcount.add(1);
          emit newCommentCreated(posts[_postid].channelId,_postid,totalComments,_commentid, comments[_commentid].subcommentcount,posts[_postid].creator,msg.sender,block.timestamp);
   
        }else{
         
          posts[_postid].commentcount=posts[_postid].commentcount.add(1);
          emit newCommentCreated(posts[_postid].channelId,_postid,totalComments,0, posts[_postid].commentcount,posts[_postid].creator,msg.sender,block.timestamp);
    ...
    function awardToComment(uint256 _id)public  onlyActiveUser(msg.sender) onlyActiveComment(_id) {
        ...
        emit awardToTheComment(posts[comments[_id].postId].channelId,comments[_id].postId,_id,comments[_id].awardCount,comments[_id].creator,msg.sender,block.timestamp  );
    ...
    function commentVoteUpSet(uint256 _id) public onlyActiveUser(_msgSender()) onlyActiveComment(_id){          
        ...
        if(commentVote[msg.sender][_id]==voteStatus.None){
            comments[_id].upvotes=comments[_id].upvotes.add(1);
            commentVote[msg.sender][_id]=voteStatus.VoteUp;
           
        }else if(commentVote[msg.sender][_id]==voteStatus.VoteDown){
             comments[_id].downvotes=comments[_id].downvotes.sub(1);
            comments[_id].upvotes=comments[_id].upvotes.add(1);
            commentVote[msg.sender][_id]=voteStatus.VoteUp;           
        }
        else{
             comments[_id].upvotes=comments[_id].upvotes.sub(1);
            commentVote[msg.sender][_id]=voteStatus.None;          
        }
      
        emit voteToComment(posts[comments[_id].postId].channelId,comments[_id].postId,_id,comments[_id].upvotes,comments[_id].downvotes, comments[_id].votecount, msg.sender,commentVote[msg.sender][_id],block.timestamp );
    }
    ...
    function commentVoteDownSet(uint256 _id) public onlyActiveUser(_msgSender()) onlyActiveComment(_id){      
         comments[_id].votecount= comments[_id].votecount.add(1); 
         if(commentVote[msg.sender][_id]==voteStatus.None){
            comments[_id].downvotes=comments[_id].downvotes.add(1);
            commentVote[msg.sender][_id]=voteStatus.VoteDown;
           
        }else if(commentVote[msg.sender][_id]==voteStatus.VoteDown){
            comments[_id].upvotes=comments[_id].downvotes.sub(1);
            commentVote[msg.sender][_id]=voteStatus.None;
...
*/
    ...
    function transferContractBalance(address payable toAddr,uint256 _amount)public payable onlyOwner(){
