...
export default function ChannelList({account,
                          chainId,
                          active ,      
                          topic,
                          selNetworks,setIsOpenSwitchNetworkModal
  ...
  async function getChannelData(newselect,searchkey ){ 
    ...
    if(selnetworkNames.length==0){
     ...
     var res= await backendPostRequest("channel/topicfilter", formData);
     if(res.status==200){
              ...
              <thead className="text-xs text-white  bg-indigo-500 dark:bg-gray-700 dark:text-gray-400">
                  <tr>
                      <th scope="col" colSpan="2" className="px-6 py-3">
                          Today's Top Communities in {topic}
                      </th>
